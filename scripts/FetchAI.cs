...
public class FetchAI : MonoBehaviour
{
    ...
    private IEnumerator PickupBall()
    {
        ...
        if (transform.gameObject.name.ToLower().Contains("deer"))
        {
            petTransform = global::Utils.FindChild(transform, "ORG-head");
        }
        else if (transform.gameObject.name.ToLower().Contains("chicken"))
        {
            petTransform = global::Utils.FindChild(transform, "CHICKEN _Head");
        }
        else if (transform.gameObject.name.ToLower().Contains("bear"))
        {
            petTransform = global::Utils.FindChild(transform, "BEAR _Head");
        }
        else
        {
